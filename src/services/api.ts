...
export async function checkMongoStatus(): Promise<MongoStatusResponse> {
  try {
    const res = await fetch(`${API_URL}/api/mongodb/status`);
...
export async function apiBuildCollections(payload: {
  ...
  try {
    const res = await fetch(
      `${API_URL}/api/mongodb/build-collections`,
      {
        method: 'POST',
        headers: {
    ...
    return {
      success: false,
      message:
        err.message || 'Build collections request failed',
      error: err.message,
    };
  }
}

export async function apiBuildCollectionsForCustomersAndProducts(
  payload?: {
    customers?: any[];
    products?: any[];
    bouquets?: any[];
  ...
  try {
    const res = await fetch(
      `${API_URL}/api/mongodb/build-customers-products`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    return {
      ...
      message:
        err.message ||
        'Build customers & products collections request failed',
      error: err.message,
    };
  }
}
...
export async function apiUpdateMongoConfig(
  ...
  success: boolean;
  message: string;
  status?: MongoStatusResponse;
  error?: string;
}> {
  try {
    const res = await fetch(
      `${API_URL}/api/mongodb/config`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    return {
      success: false,
      message:
        err.message || 'Failed to update MongoDB URI',
      error: err.message,
    };
  }
}
...
export async function apiGetCustomers(): Promise<{
  connected: boolean;
...
export async function apiGetBouquets(): Promise<{
  connected: boolean;
  bouquets: RibbonBouquet[];
}> {
  try {
    const res = await fetch(
      `${API_URL}/api/bouquets`
    );
...
export async function apiSaveBouquet(
...
export async function apiDeleteBouquet(
  id: string
): Promise<boolean> {
  try {
    const res = await fetch(
      `${API_URL}/api/bouquets/${id}`,
      {
        method: 'DELETE',
      }
    );
...
export async function apiGetOrders(): Promise<{
  connected: boolean;
  orders: Order[];
}> {
  try {
